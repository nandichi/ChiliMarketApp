...
const JWT_AUTH_URL = 'https://chili-market.com/wp-json/jwt-auth/v1';
...
class WordPressAPI {
  ...
  async testConnection(): Promise<{ success: boolean; error?: string }> {
    try {
      ...
      // Test JWT Auth plugin
      console.log('Testing JWT Auth plugin...');
      const jwtResponse = await fetch(`${this.jwtAuthURL}/token`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
      ...
      if (jwtResponse.status === 404) {
        return {
          success: false,
          error:
            'JWT Auth plugin niet gevonden. Controleer plugin installatie.',
        };
        ...
        if (
          ...
          return {
            success: false,
            error:
              'JWT Auth endpoint retourneert HTML. Plugin mogelijk niet correct geconfigureerd.',
          };
        }
        ...
        return {
          success: false,
          error:
            'JWT Auth endpoint retourneert onverwacht formaat. Controleer plugin configuratie.',
        };
        ...
        try {
          ...
          console.log(
            'JWT Auth plugin is actief (verwachte 403 voor test credentials)',
          );
          ...
          return {
            success: false,
            error: 'JWT Auth plugin retourneert ongeldige JSON response.',
          };
        }
      }
  ...
  async login(username: string, password: string): Promise<WordPressUser> {
    ...
    if (!connectionTest.success) {
      throw new Error(`Verbindingsprobleem: ${connectionTest.error}`);
    }

    try {
      console.log('Requesting JWT token...');

      const response = await fetch(`${this.jwtAuthURL}/token`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
      ...
      if (!isJsonResponse) {
        ...
        throw new Error(
          'Server retourneert onverwacht response formaat. Controleer JWT Auth plugin configuratie.',
        );
      }

      const tokenData = await response.json();
      console.log('JWT token ontvangen:', {
        user: tokenData.user_display_name,
      ...
      // Haal gebruikersdetails op met token
  ...
  async validateToken(): Promise<boolean> {
    ...
    try {
      const response = await fetch(`${this.jwtAuthURL}/token/validate`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          Authorization: `Bearer ${this.token}`,
  ...
  logout(): void {
    this.token = undefined;
    this.credentials = undefined;
    console.log('JWT logout: token cleared');
  }
  ...
  async getCurrentUser(): Promise<WordPressUser> {
    if (!this.token) {
      throw new Error('Niet ingelogd - geen JWT token');
    }

    const { data } = await this.makeRequest<WordPressUser>('/users/me');
    return data;
  }
  ...
  async debugAPI(): Promise<void> {
    ...
    try {
      ...
      // Test JWT Auth plugin
      console.log('Testing JWT Auth plugin...');
      const jwtTestResponse = await fetch(`${this.jwtAuthURL}/token`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
      ...
      if (jwtTestResponse.status === 403) {
        console.log(
          '✅ JWT Auth plugin is actief (verwachte 403 voor foute credentials)',
        );
      } else if (jwtTestResponse.status === 404) {
        console.log('❌ JWT Auth plugin niet gevonden');
      } else {
  ...
  async debugJWTAuth(): Promise<{
    ...
    try {
      const testResponse = await fetch(`${this.jwtAuthURL}/token`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
      ...
      if (testResponse.status === 404) {
        result.errorDetails =
          'JWT Auth endpoint niet gevonden - plugin niet geïnstalleerd';
      } else if (!isJson) {
        if (
          responseText.includes('<html') ||
          responseText.includes('<!DOCTYPE')
          ...
          result.errorDetails =
            'Endpoint retourneert HTML - mogelijk WordPress error of plugin niet actief';
        } else {
        ...
        try {
          ...
          console.log(
            'JWT Auth werkt correct - 403 met JSON response:',
            jsonData,
          );
          result.errorDetails =
            'Plugin werkt correct (verwachte 403 voor test credentials)';
        } catch (e) {
          result.errorDetails =
