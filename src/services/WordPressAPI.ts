...
const CUSTOM_JWT_BASE_URL = 'https://chili-market.com/wp-json/custom-jwt/v1';
...
class WordPressAPI {
  ...
  async deleteAccount(
    ...
    if (!resolvedPassword) {
      throw new Error('Wachtwoord is vereist voor accountverwijdering');
    }
    ...
    try {
      ...
      const url = `${CUSTOM_JWT_BASE_URL}/delete-account`;
      console.log('Delete account request:', url);
      ...
      const response = await fetch(url, {
        ...
        // Endpoint verwacht wachtwoordbevestiging
      ...
      if (!response.ok) {
        if (isJson) {
          ...
          try {
            message = (await response.text()).substring(0, 200);
          } catch {}
        }
        throw new Error(
          message || `Account verwijderen mislukt (${response.status})`,
        );
      }
      ...
      if (isJson) {
        try {
          const data = await response.json();
          const success = data?.success === true || response.ok;
          message = data?.message || undefined;
          return { success, message };
        } catch {
          // Valt terug op ok-status
          return { success: true };
        }
      }
      ...
      if (error instanceof Error && error.name === 'AbortError') {
        throw new Error(
          'Timeout bij verwijderen van account. Controleer je internetverbinding en probeer opnieuw.',
        );
      }
      throw error;
    }
